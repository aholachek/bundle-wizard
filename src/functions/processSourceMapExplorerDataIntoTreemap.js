...
const processData = (data, scriptsWithoutSourcemapsDict) => {
  const processed = {
    name: 'topLevel',
  ...
  Object.keys(scriptsWithoutSourcemapsDict).forEach(url => {
    processed.children.push({
      ...
      name: url,
      id: url,
      size: scriptsWithoutSourcemapsDict[url].size,
      realSize: scriptsWithoutSourcemapsDict[url].size,
      averageCoverage: scriptsWithoutSourcemapsDict[url].coveragePercent
    })
  })
  return processed
}
